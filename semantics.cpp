...
void checkVar(stack_t var)
{
	...
	if(varDefined > 0)
	{
		cout << "Smenatics error: " << varDefined << " on line: " << var.token.lineNumber << " has already been defined in the current scope at: " << stack.at(varDefined).token.lineNumber << "\n";
		exit(EXIT_FAILURE);
	}
	else
	{
...
int compareScope(stack_t var)
{
	...
	{
		cout << "Semantics error: " << getTokenDesc(var.token) << " on line: " << var.token.lineNumber << " is not found or cannot be accessed int current scope\n";
		exit(EXIT_FAILURE);
	}
}
...
void semantics(node_t *root)
{
	...
	if(root->label == "<assign>")
	{
		...
		targetFile << "STORE " << findStack(temp) << "\n";
		return;
	}
	if(root->label == "<RO>")
	{
		...
		if(operatorMap[temp.token.desc] == "LESSTHANtk")
		{
			if(!root->tokens.empty())
			{
				...
				{
					targetFile << "BRNEG branch" << repeatNum << "\n";
					return;
				}
				else if(operatorMap[temp2.token.desc] == "GREATERTHANtk")
				{
					targetFile << "BRZERO branch" << repeatNum << "\n";
					return;
				}
			}
			targetFile << "BRZNEG branch" << repeatNum << "\n";
			return;
		}
		else if(operatorMap[temp.token.desc] == "GREATERTHANtk")
		{
			if(!root->tokens.empty())
			{
				...
				{
					targetFile << "BRPOS branch" << repeatNum << "\n";
					return;
				}
			}
			targetFile << "BRZPOS branch" << repeatNum << "\n";
			return;
		}
